...
**Profile dependency:** the prompt-injection half lives in the user's PowerShell profile auto-launch
block (`Microsoft.PowerShell_profile.ps1`), which is NOT repo-tracked and not generated by
`install/`. The block must read `CLAUDE_RESTORE_PROMPT` and append it to the `claude --resume` call.
Cost: a synthetic `/switch-worktree` first turn in each restored worktree session's history, and one
eager API turn per worktree pane on restore.
